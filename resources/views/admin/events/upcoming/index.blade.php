...
@section('title', 'Admin - Upcoming Events')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-6">
        <div class="bg-white rounded-xl shadow-soft p-4 md:p-6">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-gray-500 text-xs md:text-sm">Total Events</p>
                    <p class="text-xl md:text-2xl font-bold text-gray-800" id="totalEventsStat">{{ $totalEvents ?? 0 }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-soft p-4 md:p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl md:text-2xl font-bold text-green-600" id="thisMonthStat">{{ $thisMonthCount ?? 0 }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-soft p-4 md:p-6">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-gray-500 text-xs md:text-sm">Published</p>
                    <p class="text-xl md:text-2xl font-bold text-green-600" id="publishedStat">{{ $publishedCount ?? 0 }}</p>
                </div>
                <div class="w-10 h-10 md:w-12 md:h-12 bg-green-100 rounded-full flex items-center justify-center">
        ...
        <div class="bg-white rounded-xl shadow-soft p-4 md:p-6">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-gray-500 text-xs md:text-sm">Unpublished</p>
                    <p class="text-xl md:text-2xl font-bold text-gray-800" id="unpublishedStat">{{ $unpublishedCount ?? 0 }}</p>
                </div>
                <div class="w-10 h-10 md:w-12 md:h-12 bg-gray-100 rounded-full flex items-center justify-center">
                    <i class="fas fa-eye-slash text-gray-600 text-lg md:text-xl"></i>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-bg-light p-4 rounded-lg shadow-soft border mb-6">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3">
            ...
            <div>
                ...
                <input type="text" id="searchInput" placeholder="Name, ID, Location..." 
    ...
    <div class="bg-bg-light p-4 md:p-6 rounded-lg shadow-soft border">
            ...
            <table class="min-w-full divide-y divide-gray-200" id="eventsTable">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-3 md:px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition sortable" data-sort="event_maximum_participant">
                            Participants <i class="fas fa-sort ml-1 text-gray-400"></i>
                        </th>
                        <th class="px-3 md:px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition sortable" data-sort="event_publish">
                ...
                <tbody class="bg-white divide-y divide-gray-200" id="eventsTableBody">
                    @include('admin.events.upcoming.partials.events-table', ['events' => $events])
                </tbody>
            </table>
        </div>
        ...
        <div class="mt-4 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div id="eventsPaginationLinks" class="flex justify-center space-x-2">
                @include('admin.events.upcoming.partials.pagination', ['events' => $events])
            </div>
        </div>
    </div>
@endsection
...
@push('scripts')
<script>
    let currentSortColumn = 'event_start_date';
    ...
    function fetchEvents(page = 1) {
        ...
        const fetchUrl = `{{ route('admin.events.upcoming.data') }}?${params.toString()}`;
        ...
        .then(data => {
            if (data.success) {
                ...
                if (data.stats) {
                    document.getElementById('totalEventsStat').textContent = data.stats.total || 0;
                    document.getElementById('thisMonthStat').textContent = data.stats.this_month || 0;
                    document.getElementById('publishedStat').textContent = data.stats.published || 0;
                    document.getElementById('unpublishedStat').textContent = data.stats.unpublished || 0;
                }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('resetFilters')?.addEventListener('click', function() {
            ...
            currentSortColumn = 'event_start_date';
            currentSortDirection = 'asc';
