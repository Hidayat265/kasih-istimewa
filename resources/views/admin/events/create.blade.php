...
<section class="py-8 md:py-12">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-2xl shadow-soft overflow-hidden">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    <div>
                        <h2 class="text-lg md:text-xl font-bold text-gray-800">
                            <i class="fas fa-calendar-plus text-primary mr-2"></i> Create New Event (Admin)
                        </h2>
                        <p class="text-xs md:text-sm text-gray-500 mt-1">Please fill in all required fields marked with <span class="text-red-500">*</span></p>
                    </div>
                    <a href="{{ route('admin.pendingevent') }}" 
                       class="inline-flex items-center px-3 py-1.5 bg-gray-500 hover:bg-gray-600 text-white text-xs font-medium rounded-lg transition">
                        <i class="fas fa-arrow-left mr-1"></i> Back to Pending Events
                    </a>
                </div>
            </div>
            ...
            <!-- Form Body -->
            <form action="{{ route('admin.events.store') }}" method="POST" enctype="multipart/form-data" class="p-4 md:p-6" id="eventForm">
                @csrf
                    ...
                    <div class="bg-blue-50 border-l-4 border-blue-400 p-4 rounded">
                        <div class="flex">
                            ...
                            <p class="text-sm text-blue-700 ml-3">
                                ...
                                You can create events for any date (no date restrictions).
                            </p>
                        </div>
                    </div>
...
// Add location validation to basic info fields
basicInfoFields.forEach(f => {
...
function switchTab(tabId) {
    ...
    tabs.forEach(t => {
        ...
        t.classList.remove('active'); // Remove any custom active class
...
document.querySelectorAll('.next-tab').forEach(btn => {
    btn.addEventListener('click', function(e) {
        ...
        // Check for session conflicts when moving from Tab 2 to Tab 3
        if (btnId === 'nextToTab3') {
            // Check if there are conflicts
            const hasConflict = checkForConflicts();
            
            if (hasConflict) {
                // Build conflict details HTML with clickable event links
                let conflictHtml = '';
                ...
                // Show warning popup with details
                Swal.fire({
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        // User confirmed, proceed to next tab
        ...
        // No conflicts or different button, proceed normally
        const cur = btn.closest('.tab-content');
        const next = cur ? cur.nextElementSibling : null;
        if (next && next.classList.contains('tab-content')) {
            switchTab(next.id);
...
function checkForConflicts() {
    ...
    // Get conflicting events for display
    const allBookedDates = Object.keys(bookedSessionsByDate);
    ...
    if (isSameDate) {
        ...
        if (overlapping.length > 0) {
            ...
            // Build HTML for same date conflicts with clickable events
            ...
            // Add clickable event links
            detailsHtml += `<div class="mt-2 text-xs text-blue-600">
                <i class="fas fa-info-circle"></i> <span class="font-medium">Click to view conflicting events:</span><br>`;
            
            // Fetch and display conflicting events
            const conflictingEventIds = getConflictingEventIdsForDate(startDate);
            if (conflictingEventIds.length > 0) {
                conflictingEventIds.forEach(eventId => {
                    detailsHtml += `<a href="/admin/events/${eventId}" target="_blank" 
                                    class="text-primary hover:underline block mt-1">
                        📄 Event: ${eventId}
                    </a>`;
                });
            } else {
                detailsHtml += `<span class="text-gray-400">No specific event details available</span>`;
            }
            detailsHtml += `</div>`;
        ...
        if (overlappingEnd.length > 0) {
            ...
            detailsHtml += `<div class="mt-2 text-xs text-blue-600">
                <i class="fas fa-info-circle"></i> <span class="font-medium">Click to view conflicting events:</span><br>`;
            const conflictingEventIds = getConflictingEventIdsForDate(endDate);
            if (conflictingEventIds.length > 0) {
                conflictingEventIds.forEach(eventId => {
                    detailsHtml += `<a href="/admin/events/${eventId}" target="_blank" 
                                    class="text-primary hover:underline block mt-1">
        ...
        while (cur < fin) {
            ...
            if (booked.length > 0) {
                ...
                detailsHtml += `<div class="mt-2 text-xs text-blue-600">
                    <i class="fas fa-info-circle"></i> <span class="font-medium">Click to view conflicting events:</span><br>`;
                const conflictingEventIds = getConflictingEventIdsForDate(ds);
                if (conflictingEventIds.length > 0) {
                    conflictingEventIds.forEach(eventId => {
                        detailsHtml += `<a href="/admin/events/${eventId}" target="_blank" 
                                        class="text-primary hover:underline block mt-1">
...
// Helper function to get conflicting event IDs for a date
function getConflictingEventIdsForDate(date) {
    // This would ideally fetch from the server
    // For now, return a sample or empty array
    // You can implement an AJAX call to fetch actual conflicting events
    const events = [];
    // Check if there are any booked sessions for this date
    const booked = getBookedSessionsForDate(date);
    if (booked.length > 0) {
        // You can fetch actual event IDs from the server here
        // For now, we'll return a placeholder
        // In production, you'd make an AJAX call to get the actual events
        return ['EVT-0001']; // Placeholder
    }
    return events;
}
...
function updateSessionAvailability(bookedMap) {
    ...
    // Style start session boxes - NO WARNING TEXT, just yellow border
    document.querySelectorAll('.start-session-box').forEach(box => {
        ...
        box.classList.toggle('border-yellow-400', booked);
        box.classList.toggle('bg-yellow-50', booked);
        ...
        if (statusDiv) {
            statusDiv.innerHTML = ''; // Remove warning text
    ...
    // Style end session boxes - NO WARNING TEXT
...
function updateSameDateSelectionUI() {
    ...
    document.querySelectorAll('.start-session-box').forEach(box => {
        ...
        box.classList.toggle('border-gray-300', !active.has(s) && !box.classList.contains('border-yellow-400'));
        box.classList.toggle('bg-white',        !active.has(s) && !box.classList.contains('bg-yellow-50'));
    });
...
document.querySelectorAll('.start-session-box').forEach(box => {
    box.addEventListener('click', () => {
        ...
        const isSameDate = startDate && endDate && startDate === endDate;
        selectSession(session, isSameDate, box);
    });
});

function selectSession(session, isSameDate, box) {
    if (isSameDate) {
...
document.querySelectorAll('.end-session-box').forEach(box => {
    box.addEventListener('click', () => {
        ...
        const session = box.getAttribute('data-session');
        selectEndSession(session, box);
    });
});

function selectEndSession(session, box) {
...
// No min date restriction - admins can select any date
if (startDateInput) {
...
async function setLocation(lat, lng) {
    ...
    Swal.fire({
        ...
        text: 'The map address has been saved.',
        confirmButtonColor: '#554994', timer: 1500, showConfirmButton: false
    });
}
